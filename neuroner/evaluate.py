...
def assess_model(
    ...
    for f1_average_style in ["weighted", "micro", "macro"]:
        results["f1_score"][f1_average_style] = (
            sklearn.metrics.f1_score(
                y_true, y_pred, average=f1_average_style, labels=labels
            )
            * 100
        )
    results["f1_score"]["per_label"] = [
        x * 100
        for x in sklearn.metrics.precision_recall_fscore_support(
            y_true, y_pred, average=None, labels=labels
        )[2].tolist()
    ]

    confusion_matrix = sklearn.metrics.confusion_matrix(
        y_true, y_pred, labels=labels_with_o
    )
...
def save_results(results, stats_graph_folder):
    ...
    json.dump(
        results,
        open(os.path.join(stats_graph_folder, "results.json"), "w"),
        indent=4,
        sort_keys=True,
    )
...
def plot_f1_vs_epoch(results, stats_graph_folder, metric, parameters, from_json=False):
    ...
    for dataset_type in dataset_types:
        ...
        results[dataset_type]["epoch_for_best_{0}".format(metric)] = int(
            np.asarray(f1).argmax()
        )
        ...
        plot_handles.extend(
            plt.plot(
                ...
                label=dataset_type
                + " (max: {0:.4f})".format(
                    results[dataset_type]["best_{0}".format(metric)]
                ),
            )
        )
    ...
    plt.axvline(
        x=best_epoch, color="k", linestyle=":"
    )  # Add a vertical line at best epoch for valid
    for dataset_type in dataset_types:
        best_score_based_on_valid = f1_all[dataset_type][best_epoch]
        results[dataset_type][
            "best_{0}_based_on_valid".format(metric)
        ] = best_score_based_on_valid
        if dataset_type == "test":
            plot_handles.append(
                plt.axhline(
                    y=best_score_based_on_valid,
                    label=dataset_type
                    + " (best: {0:.4f})".format(best_score_based_on_valid),
                    color="k",
                    linestyle=":",
                )
            )
            ...
            plt.axhline(
                y=best_score_based_on_valid,
                label="{0:.4f}".format(best_score_based_on_valid),
                color="k",
                linestyle=":",
            )
    ...
    plt.savefig(
        os.path.join(
            stats_graph_folder,
            "{0}_vs_epoch_for_all_classes.{1}".format(
                metric, parameters["plot_format"]
            ),
        )
    )
    plt.close()
...
def remap_labels(y_pred, y_true, dataset, evaluation_mode="bio"):
    ...
    return (
        ...
        new_label_indices,
        new_label_names,
        new_label_indices_with_o,
        new_label_names_with_o,
    )
...
def evaluate_model(
    ...
    result_update["time_elapsed_since_train_start"] = (
        time.time() - results["execution_details"]["train_start"]
    )
    ...
    for dataset_type in ["train", "valid", "test"]:
        ...
        results["epoch"][epoch_number][0][dataset_type]["f1_conll"]["micro"] = results[
            "epoch"
        ][epoch_number][0][dataset_type]["conll"]["all"]["f1"]
        if parameters["main_evaluation_mode"] == "conll":
            ...
            results["epoch"][epoch_number][0][dataset_type]["accuracy_score"] = results[
                "epoch"
            ][epoch_number][0][dataset_type]["conll"]["all"]["accuracy"]
            utils_plots.plot_classification_report(
            ...
            plt.savefig(
                os.path.join(
                    stats_graph_folder,
                    "classification_report_for_epoch_{0:04d}_in_{1}_conll_evaluation.{3}".format(
                        epoch_number,
                        dataset_type,
                        evaluation_mode,
    ...
    if (
        ...
        and "train" in output_filepaths.keys()
    ...
    results["execution_details"]["train_duration"] = (
        time.time() - results["execution_details"]["train_start"]
    )
