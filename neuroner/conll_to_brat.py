...
def generate_reference_text_file_for_conll(
    conll_input_filepath, conll_output_filepath, text_folder
):
    """
    ...
    conll_file = codecs.open(conll_input_filepath, "r", "ISO-8859-1")
    ...
    text_base_filename = "{0}_text_{1}".format(
        dataset_type, str(document_count).zfill(5)
    )
    for line in conll_file:
        ...
        if "-DOCSTART-" in split_line[0]:
            ...
            if len(text) != 0:
                with codecs.open(
                    os.path.join(text_folder, "{0}.txt".format(text_base_filename)),
                    "w",
                    "ISO-8859-1",
                ) as f:
                    f.write(text)
        ...
        new_conll_string += (
            " ".join([token, text_base_filename, str(start), str(end)] + split_line[1:])
            + "\n"
        )
    if len(text) != 0:
        with codecs.open(
    ...
    with codecs.open(conll_output_filepath, "w", "ISO-8859-1") as f:
...
def check_compatibility_between_conll_and_brat_text(conll_filepath, brat_folder):
    ...
    print(
        "Checking compatibility between CONLL and BRAT for {0} set ... ".format(
            dataset_type
        ),
        end="",
    )
    conll_file = codecs.open(conll_filepath, "r", "ISO-8859-1")
    ...
    for line in conll_file:
        ...
        if filename != previous_filename:
            ...
            with codecs.open(text_filepath, "r", "ISO-8859-1") as f:
                text = f.read()
...
def output_entities(
    ...
    output_filepath = os.path.join(
        brat_output_folder, "{0}.ann".format(previous_filename)
    )
    if not overwrite:
        # Avoid overriding existing annotation
        if os.path.exists(output_filepath) and os.path.getsize(output_filepath) > 0:
            raise AssertionError(
                "The annotation already exists at: {0}".format(output_filepath)
            )
    # Write the entities to the annotation file
    with codecs.open(output_filepath, "w", "ISO-8859-1") as output_file:
    ...
    if text_filepath != os.path.join(
        brat_output_folder, os.path.basename(text_filepath)
    ):
        shutil.copy(text_filepath, brat_output_folder)


def conll_to_brat(
    conll_input_filepath,
    ...
    if (
        not os.path.exists(brat_original_folder)
        or len(glob.glob(os.path.join(brat_original_folder, "*.txt"))) == 0
    ):
        assert conll_input_filepath != conll_output_filepath
        generate_reference_text_file_for_conll(
            conll_input_filepath, conll_output_filepath, brat_original_folder
        )

    utils.create_folder_if_not_exists(brat_output_folder)
    conll_file = codecs.open(conll_output_filepath, "r", "ISO-8859-1")
    ...
    for line in conll_file:
        ...
        if filename != previous_filename:
            ...
            text_filepath = os.path.join(
                brat_original_folder, "{0}.txt".format(filename)
            )
            with codecs.open(text_filepath, "r", "ISO-8859-1", errors="ignore") as f:
                text = f.read()
            previous_token_label = "O"
            previous_filename = filename
            entity_id = 1
