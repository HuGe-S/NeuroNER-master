...
def _clean_param_dtypes(param):
    ...
    for k, v in param.items():
        ...
        if k in [
            ...
            "debug",
            "verbose",
            "reload_character_embeddings",
            "reload_character_lstm",
            "reload_token_embeddings",
            "reload_token_lstm",
            "reload_feedforward",
            "reload_crf",
            "check_for_lowercase",
...
def load_parameters(**kwargs):
    ...
    if param["use_pretrained_model"]:
        ...
        pretrain_path = os.path.join(param["pretrained_model_folder"], "parameters.ini")
...
def get_valid_dataset_filepaths(parameters):
    ...
    for dataset_type in ["train", "valid", "test", "deploy"]:
        ...
        # Conll file exists
        if (
            os.path.isfile(dataset_filepaths[dataset_type])
            and os.path.getsize(dataset_filepaths[dataset_type]) > 0
        ):

            # Brat text files exist
            if (
                os.path.exists(dataset_brat_folders[dataset_type])
                and len(
                    glob.glob(os.path.join(dataset_brat_folders[dataset_type], "*.txt"))
                )
                > 0
            ):
                ...
                conll_to_brat.check_compatibility_between_conll_and_brat_text(
                    dataset_filepaths[dataset_type], dataset_brat_folders[dataset_type]
                )
...
def check_param_compatibility(parameters, dataset_filepaths):
    ...
    if parameters["train_model"]:
        if "train" not in dataset_filepaths or "valid" not in dataset_filepaths:
            msg = """If train_model is set to True, both train and valid set must exist 
                in the specified dataset folder: {0}""".format(
                parameters["dataset_text_folder"]
            )
            raise IOError(msg)
        ...
        if "train" in dataset_filepaths and "valid" in dataset_filepaths:
            msg = """WARNING: train and valid set exist in the specified dataset folder, 
                but train_model is set to FALSE: {0}""".format(
                parameters["dataset_text_folder"]
            )
            print(msg)
    ...
    # if not parameters['train_model'] and not parameters['use_pretrained_model']:
        ...
        raise ValueError(
            "At least one of train_model and use_pretrained_model must be set to True."
        )
    ...
    if parameters["gradient_clipping_value"] < 0:
        parameters["gradient_clipping_value"] = abs(
...
class NeuroNER(object):
    ...
    def __init__(self, **kwargs):
        ...
        ) = self._get_valid_dataset_filepaths(self.parameters)
        self._check_param_compatibility(self.parameters, self.dataset_filepaths)

        # Load dataset
        self.modeldata = dataset.Dataset(
            verbose=self.parameters["verbose"], debug=self.parameters["debug"]
        )
        token_to_vector = self.modeldata.load_dataset(
            self.dataset_filepaths, self.parameters
        )
        ...
        with self.sess.as_default():
            ...
            if self.parameters["use_pretrained_model"]:
                ...
                self.model.load_pretrained_token_embeddings(
                    self.sess, self.modeldata, self.parameters, token_to_vector
                )
    ...
    def _get_valid_dataset_filepaths(
        self, parameters, dataset_types=["train", "valid", "test", "deploy"]
    ):
        """
        ...
        for dataset_type in dataset_types:
            ...
            if (
                ...
                if (
                    ...
                    and len(
                        glob.glob(
                            os.path.join(dataset_brat_folders[dataset_type], "*.txt")
                        )
                    )
                    > 0
                ):
    ...
    def fit(self):
        ...
        tensorboard_character_embeddings.tensor_name = (
            model.character_embedding_weights.name
        )
        ...
        try:
            while True:
                ...
                if epoch_number != 0:
                    ...
                    for sequence_number in sequence_numbers:
                        transition_params_trained = train.train_step(
                            sess, modeldata, sequence_number, model, parameters
                        )
                        step += 1
                        if step % 10 == 0:
                            print(
                                "Training {0:.2f}% done".format(
                ...
                print(
                    "Training completed in {0:.2f} seconds".format(
                ...
                if parameters["use_pretrained_model"] and not parameters["train_model"]:
                    conll_to_brat.output_brat(
                        output_filepaths, dataset_brat_folders, stats_graph_folder
                    )
                    break
                ...
                model.saver.save(
                    sess,
                    os.path.join(
                        model_folder, "model_{0:05d}.ckpt".format(epoch_number)
                    ),
                )
                ...
                # Early stop
                valid_f1_score = results["epoch"][epoch_number][0]["valid"]["f1_score"][
                    "micro"
                ]
    ...
    def predict(self, text):
        ...
        for filepath in glob.glob(
            os.path.join(
                self.parameters["dataset_text_folder"], "{0}*".format(dataset_type)
            )
        ):
