...
def bidirectional_LSTM(
    ...
    with tf.variable_scope("bidirectional_LSTM"):
        if sequence_length == None:
            ...
            sequence_length = tf.expand_dims(
                sequence_length, axis=0, name="sequence_length"
            )
        else:
        ...
        for direction in ["forward", "backward"]:
            with tf.variable_scope(direction):
                ...
                initial_state[direction] = tf.contrib.rnn.LSTMStateTuple(
                    c_states, h_states
                )
        ...
        if output_sequence == True:
            ...
            output = tf.concat(
                [outputs_forward, outputs_backward], axis=2, name="output_sequence"
            )
        else:
            # # max pooling
            # outputs_forward, outputs_backward = outputs
            ...
            output = tf.concat(
                [final_states_forward[1], final_states_backward[1]],
                axis=1,
                name="output",
            )
...
class EntityLSTM(object):
    ...
    def __init__(self, dataset, parameters):
        ...
        self.input_token_indices = tf.placeholder(
            tf.int32, [None], name="input_token_indices"
        )
        ...
        with tf.variable_scope("dropout"):
            token_lstm_input_drop = tf.nn.dropout(
                token_lstm_input, self.dropout_keep_prob, name="token_lstm_input_drop"
            )
            ...
            token_lstm_input_drop_expanded = tf.expand_dims(
                token_lstm_input_drop, axis=0, name="token_lstm_input_drop_expanded"
            )

            if self.verbose:
                print(
                    "token_lstm_input_drop_expanded: {0}".format(
        ...
        with tf.variable_scope("feedforward_after_lstm") as vs:
            ...
            outputs = tf.nn.xw_plus_b(
                token_lstm_output_squeezed, W, b, name="output_before_tanh"
            )
        ...
        with tf.variable_scope("feedforward_before_crf") as vs:
            ...
            b = tf.Variable(
                tf.constant(0.0, shape=[dataset.number_of_classes]), name="bias"
            )
            ...
            with tf.variable_scope("crf") as vs:
                ...
                unary_scores_with_start_and_end = tf.concat(
                    [
                        self.unary_scores,
                        tf.tile(
                            tf.constant(small_score, shape=[1, 2]), [sequence_length, 1]
                        ),
                    ],
                    1,
                )
                ...
                sequence_lengths = tf.expand_dims(
                    sequence_length, axis=0, name="sequence_lengths"
                )
                ...
                if self.verbose:
                    ...
                    print(
                        "input_label_indices_flat_batch: {0}".format(
                ...
                self.loss = tf.reduce_mean(
                    -log_likelihood, name="cross_entropy_mean_loss"
                )
        ...
        # Do not use CRF layer
            ...
            with tf.variable_scope("accuracy"):
                correct_predictions = tf.equal(
                    self.predictions, tf.argmax(self.input_label_indices_vector, 1)
                )
    ...
    def define_training_procedure(self, parameters):
        ...
        if parameters["optimizer"] == "adam":
            ...
            raise ValueError(
                "The lr_method parameter must be either adadelta, adam or sgd."
            )
    ...
    def load_pretrained_token_embeddings(
        self, sess, dataset, parameters, token_to_vector=None
    ):
        """ """
        ...
        for token in dataset.token_to_index.keys():
            if token in token_to_vector.keys():
                ...
                parameters["check_for_lowercase"]
                and token.lower() in token_to_vector.keys()
            ):
                initial_weights[dataset.token_to_index[token]] = token_to_vector[
                    token.lower()
                ]
                number_of_token_lowercase_found += 1
            elif (
                parameters["check_for_digits_replaced_with_zeros"]
                and re.sub(r"\d", "0", token) in token_to_vector.keys()
            ):
                initial_weights[dataset.token_to_index[token]] = token_to_vector[
                    re.sub(r"\d", "0", token)
                ]
                ...
                parameters["check_for_lowercase"]
                and parameters["check_for_digits_replaced_with_zeros"]
                and re.sub("\d", "0", token.lower()) in token_to_vector.keys()
            ):
                initial_weights[dataset.token_to_index[token]] = token_to_vector[
                    re.sub(r"\d", "0", token.lower())
                ]
        ...
        print(
            "number_of_token_original_case_found: {0}".format(
                number_of_token_original_case_found
            )
        )
        print(
            "number_of_token_lowercase_found: {0}".format(
                number_of_token_lowercase_found
            )
        )
        print(
            "number_of_token_digits_replaced_with_zeros_found: {0}".format(
                number_of_token_digits_replaced_with_zeros_found
            )
        )
        print(
            "number_of_token_lowercase_and_digits_replaced_with_zeros_found: {0}".format(
                number_of_token_lowercase_and_digits_replaced_with_zeros_found
            )
        )
        print(
            "number_of_loaded_word_vectors: {0}".format(number_of_loaded_word_vectors)
        )
    ...
    def load_embeddings_from_pretrained_model(
        self,
        sess,
        dataset,
        pretraining_dataset,
        ...
        start_time = time.time()
        print(
            "Load {0} embeddings from pretrained model... ".format(embedding_type),
            end="",
            flush=True,
        )
        ...
        if embedding_type == "token":
            initial_weights[dataset.UNK_TOKEN_INDEX] = pretrained_embedding_weights[
                pretraining_dataset.UNK_TOKEN_INDEX
            ]
        elif embedding_type == "character":
            initial_weights[
                dataset.PADDING_CHARACTER_INDEX
            ] = pretrained_embedding_weights[
        ...
        for index, string in index_to_string.items():
            ...
            if string in pretraining_string_to_index.keys():
                initial_weights[index] = pretrained_embedding_weights[
    ...
    def restore_from_pretrained_model(
        self, parameters, dataset, sess, token_to_vector=None
    ):
        """ """
        try:
            pretraining_dataset = pickle.load(
                open(
                    os.path.join(
                        parameters["pretrained_model_folder"], "dataset.pickle"
                    ),
                    "rb",
                )
            )
        ...
        if (
            pretraining_dataset.index_to_token == dataset.index_to_token
            and pretraining_dataset.index_to_character == dataset.index_to_character
        ):
