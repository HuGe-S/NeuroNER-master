...
def plot_classification_report(
    ...
    if from_conll_json:
        for label in sorted(classification_report.keys()):
            ...
            plotMat.append(
                [
                    float(classification_report[label][x])
...
def plot_hist(sequence, xlabel, ylabel, title, graph_path):
    ...
    step = 1
    y, x = np.histogram(
        sequence, bins=np.linspace(xmin, xmax, (xmax - xmin + 1) / step)
    )
...
def plot_barh(x, y, xlabel, ylabel, title, graph_path):
    ...
    plt.ylabel(ylabel)
    plt.savefig(
        graph_path, dpi=300, format="png", bbox_inches="tight"
...
def plot_precision_recall_curve(recall, precision, graph_path, title):
    ...
    plt.savefig(
        graph_path, dpi=600, format="pdf", bbox_inches="tight"
    )  # use format='svg' or 'pdf' for vectorial pictures
    plt.close()
