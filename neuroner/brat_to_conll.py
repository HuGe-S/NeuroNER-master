    ...
    for span in document.sents:
        ...
        for token in sentence:
            ...
            (
                token_dict["start"],
                token_dict["end"],
            ) = get_start_and_end_offset_of_token_from_spacy(token)
...
def get_entities_from_brat(text_filepath, annotation_filepath, verbose=False):
    # load text
    with codecs.open(text_filepath, "r", "UTF-8", errors="ignore") as f:
        text = f.read()
    if verbose:
        print("\ntext:\n{0}\n".format(text))
    ...
    entities = []
    with codecs.open(annotation_filepath, "r", "UTF-8", errors="ignore") as f:
        for line in f.read().splitlines():
            ...
            if id_anno[0] == "T":
                ...
                if utils_nlp.replace_unicode_whitespaces_with_ascii_whitespace(
...
def check_brat_annotation_and_text_compatibility(brat_folder):
    ...
    print(
        "Checking the validity of BRAT-formatted {0} set... ".format(dataset_type),
        end="",
    )
    ...
    for text_filepath in text_filepaths:
        ...
        if not os.path.exists(annotation_filepath):
            raise IOError(
                "Annotation file does not exist: {0}".format(annotation_filepath)
            )
        ...
        for sentence in sentences:
            ...
            for token in sentence:
                ...
                for entity in entities:
                    if (
                        entity["start"] <= token["start"] < entity["end"]
                        or entity["start"] < token["end"] <= entity["end"]
                        or token["start"]
                        < entity["start"]
                        < entity["end"]
                        < token["end"]
