...
def load_tokens_from_pretrained_token_embeddings(parameters):
    file_input = codecs.open(
        parameters["token_pretrained_embedding_filepath"], "r", "UTF-8"
    )
    count = -1
    tokens = set()
...
def is_token_in_pretrained_embeddings(token, all_pretrained_tokens, parameters):
    return (
        token in all_pretrained_tokens
        or parameters["check_for_lowercase"]
        and token.lower() in all_pretrained_tokens
        or parameters["check_for_digits_replaced_with_zeros"]
        and re.sub("\d", "0", token) in all_pretrained_tokens
        or parameters["check_for_lowercase"]
        and parameters["check_for_digits_replaced_with_zeros"]
        and re.sub("\d", "0", token.lower()) in all_pretrained_tokens
    )
...
def get_parsed_conll_output(conll_output_filepath):
    conll_output = [
        ...
        for l in codecs.open(conll_output_filepath, "r", "utf8")
    ]
    parsed_output = {}
...
def end_current_entity(
    previous_label_without_bio, current_entity_length, new_labels, i
):
    """
...
def bio_to_bioes(labels):
    ...
    for i, label in enumerate(labels):
        ...
        if current_entity_length > 0 and (
            label[:2] in ["B-", "O"]
            or label[:2] == "I-"
            and previous_label_without_bio != label_without_bio
        ):
            end_current_entity(
                previous_label_without_bio, current_entity_length, new_labels, i
            )
    ...
    end_current_entity(
        previous_label_without_bio, current_entity_length, new_labels, i + 1
    )
    return new_labels
...
def check_validity_of_conll_bioes(bioes_filepath):
    ...
    input_conll_file = codecs.open(bioes_filepath, "r", "ISO-8859-1")
    labels_bioes = []
    labels_bio = []
...
def convert_conll_from_bio_to_bioes(input_conll_filepath, output_conll_filepath):
    if os.path.exists(output_conll_filepath):
        if check_validity_of_conll_bioes(output_conll_filepath):
            return
    dataset_type = utils.get_basename_without_extension(input_conll_filepath).split(
        "_"
    )[0]
