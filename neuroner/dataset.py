...
class Dataset(object):
    ...
    def _parse_dataset(self, dataset_filepath):
        ...
        new_label_sequence = []
        if dataset_filepath:
            f = codecs.open(dataset_filepath, "r", "ISO-8859-1")
            for line in f:
                line_count += 1
    ...
    def _convert_to_indices(self, dataset_types):
        ...
        if self.verbose:
            print(
                "token_lengths['train'][0][0:10]: {0}".format(
        ...
        if self.verbose:
            print(
                "character_indices['train'][0][0:10]: {0}".format(
        ...
        if self.verbose:
            ...
            print(
                "character_indices_padded['train'][0][0:10]: {0}".format(
        ...
        if self.verbose:
            print(
                "label_vector_indices['train'][0:2]: {0}".format(
                    label_vector_indices["train"][0:2]
                )
            )
        if self.verbose:
            print(
                "len(label_vector_indices['train']): {0}".format(
    ...
    def update_dataset(self, dataset_filepaths, dataset_types):
        ...
        for dataset_type in dataset_types:
            (
                self.labels[dataset_type],
                self.tokens[dataset_type],
                _,
                _,
                _,
        ...
        (
            ...
            characters,
            label_vector_indices,
        ) = self._convert_to_indices(dataset_types)
    ...
    def load_dataset(self, dataset_filepaths, parameters, token_to_vector=None):
        ...
        for dataset_type in ["train", "valid", "test", "deploy"]:
            (
                labels[dataset_type],
                tokens[dataset_type],
                token_count[dataset_type],
                label_count[dataset_type],
        ...
        token_count["all"] = {}
        for token in (
            list(token_count["train"].keys())
            + list(token_count["valid"].keys())
            + list(token_count["test"].keys())
            + list(token_count["deploy"].keys())
        ):
            token_count["all"][token] = (
                token_count["train"][token]
        ...
        character_count["all"] = {}
        for character in (
            list(character_count["train"].keys())
            ...
            character_count["all"][character] = (
                character_count["train"][character]
                + character_count["valid"][character]
                + character_count["test"][character]
        ...
        label_count["all"] = utils.order_dictionary(
            label_count["all"], "key", reverse=False
        )
        ...
        if self.verbose:
            print(
                "len(token_count['train'].keys()): {0}".format(
        ...
        for token, count in token_count["all"].items():
            ...
            if (
                ...
                if self.verbose:
                    print(
                        "re.sub('\d', '0', token.lower()): {0}".format(
        ...
        if self.verbose:
            print(
                "len(infrequent_token_indices): {0}".format(
        ...
        if self.verbose:
            print(
                "token_count['train'][0:10]: {0}".format(
        ...
        character_to_index = utils.order_dictionary(
            character_to_index, "value", reverse=False
        )
        ...
        (
            ...
            characters,
            label_vector_indices,
        ) = self._convert_to_indices(dataset_filepaths.keys())
        ...
        print(self.unique_labels_of_interest)
        ...
        if self.verbose:
            print(
                "self.unique_labels_of_interest: {0}".format(
                    self.unique_labels_of_interest
                )
            )
        if self.verbose:
            print(
                "self.unique_label_indices_of_interest: {0}".format(
                    self.unique_label_indices_of_interest
                )
            )
